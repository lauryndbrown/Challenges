...
class TwentyFortyEight:
    ...
    def __init__(self, grid_height, grid_width):
        ...
        self.reset()
        self._initial_tiles={}
        self._initial_tiles[UP]=[(0, col) for col in range(self._width)]
        self._initial_tiles[DOWN]=[(self._height-1,col) for col in range(self._width)]
        self._initial_tiles[LEFT]=[(row,0) for row in range(self._height)]
        self._initial_tiles[RIGHT]=[(row,self._width-1) for row in range(self._height)]
    ...
    def move(self, direction):
        ...
        """
        tiles_moved = False
        for initial_pos in self._initial_tiles[direction]:
            row = initial_pos[0]
            col = initial_pos[1]
    ...
    def copy_line_to_grid(self, row, col, direction, line):
        """
        Copy all values to the row and col given 
        """
    ...
    def build_temp_line(self, row, col, direction):
        """
        Build a temporary list of values in the direction given
        """
        temp_line = []
    ...
    def is_in_bounds(self, row, col):
        """
        Check if row and col are in the bounds of the grid
        """
        return  row<self._height and row>=0 and col<self._width and col>=0
    ...
    def new_tile(self):
        ...
        for row in range(self._height):
            for col in range(self._width):
                if self._grid[row][col]==EMPTY_POS:
